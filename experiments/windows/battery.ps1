...
WriteLf "$mountRoot\abi.py" @'
import ctypes
k = ctypes.CDLL(None, use_errno=True)
k.syscall.argtypes = [ctypes.c_long, ctypes.c_void_p, ctypes.c_size_t, ctypes.c_uint]
k.syscall.restype = ctypes.c_long
r = k.syscall(444, None, 0, 16)
print("abi-version", r, "errno", ctypes.get_errno())
'@
$kernel = Ssh "uname -r; sudo mount -t securityfs none /sys/kernel/security 2>&1; sudo ls /sys/kernel/security/ 2>&1; cat /sys/kernel/security/lsm 2>/dev/null; stat -fc %T /sys/fs/cgroup; cat /proc/sys/user/max_user_namespaces; ls /init 2>&1 | head -1; python3 /mnt/c/errand-probe-mount/abi.py 2>&1 | tail -1; ls /proc/sys/fs/binfmt_misc/ 2>/dev/null"
